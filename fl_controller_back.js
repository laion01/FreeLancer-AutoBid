		...
		flSock.onmessage = function(d) {
		    ...
		    if ((e.channel=='subscribe') && (e.body=='OK')){
                ...
                if(e.body.type == 'project') {
					...
					if (new RegExp(keywords.join("|")).test(data.jobString) && 
						(data.currencyCode == "USD" || data.currencyCode == "EUR" || data.currencyCode == "GBP" || data.currencyCode == "AUD" || data.currencyCode == "CAD")) {
						if(((data.projIsHourly && data.minbudget >= 15) || (!data.projIsHourly && data.minbudget >= 250)) && (data.minbudget < 1500)) {						

							var b = (data.minbudget*1 + data.maxbudget*1)/2;
							var	budget = b / 0.9;
							var period = 30;

							if(!data.projIsHourly) {
								if (data.minbudget >= 250 && data.maxbudget <= 750) {
									period = 10;
								} else if (data.minbudget >= 750 && data.maxbudget <= 1500) {
									period = 20;
								} else if (data.minbudget >= 1500 && data.maxbudget <= 3000) {
									period = 30;
								} else if (data.minbudget >= 3000 && data.maxbudget <= 5000) {
									period = 50;
								} else if (data.minbudget >= 5000) {
									period = data.maxbudget / 100;
								}
							} else {
								period = 40;
							}

							var myDescription = "Hello,I am a mobile and web developer. I love mobile app programming and I have a large portfolio of mobile apps built specifically on the ios and android platforms. Below you can see a portfolio of my past projects. http://bit.ly/2ojG0gY Please explain me your job detail. Thank you and I look forward to the opportunity to work with you.";
